...
$sql = "SELECT worked_assign_files.subDate, worked_assign_files.studentId,studentFirstName,studentMiddleName,studentLastName FROM worked_assign_files  
INNER JOIN students ON students.studentId = worked_assign_files.studentId  
WHERE worked_assign_files.assignmentId = $assignmentId ORDER BY worked_assign_files.subDate ASC";
$stmt = $pdo->query($sql);
$workedAssignmentRows = $stmt->fetchAll(PDO::FETCH_ASSOC);

$sql = "SELECT assignTitle,courses.* FROM assignments  INNER JOIN courses ON courses.courseId = assignments.courseId  
            ...
            <div class="main-panel">
                <div class="content-wrapper">
                    ...
                    <div class="row" id="proBanner">
                        <div class="col-12">
                            <p style="color:#1c2a33; font-size:20px">
                                Assignment Title: <?php echo $row['assignTitle'];?>  
                            </p>
                            <p style="color:#1c2a33; font-size:20px">
                                Course: <?php echo $row['courseName'];?>
                            </p>
                            <?php 
                                if($workedAssignmentRows){
                            ?>
                            ...
                            <tbody>
                                <?php
                                    $s_no = 1;
                                    foreach($workedAssignmentRows as $workedAssignmentRow){
                                        $date = DateTime::createFromFormat('Y-m-d H:i:s', $workedAssignmentRow['subDate']);
                                        $subDate = $date->format('d-m-Y H:i:s');
                                        echo '<tr>';
                                            echo '<td>';
                                                echo $s_no;
                                            echo '</td>';
                                            echo '<td>';
                                                echo $workedAssignmentRow['studentFirstName']." ".$workedAssignmentRow['studentLastName'];
                                            echo '</td>';
                                            echo '<td>';
                                                echo $workedAssignmentRow['studentId'];
                                            echo '</td>';
                                            echo '<td>';
                                                echo $subDate;
                                            echo '</td>';
                                            echo '<td>';
                                                echo '<a href="lect_mark_assign.php?studentId='.$workedAssignmentRow['studentId'].'&assignmentId='.$assignmentId.'" class="btn btn-primary btn-sm">View</a>';
                        ...
                        <?php 
                                ...
                                echo'
                                    <center>
                                        ...
                                        <a href="lect_view_assigns.php" class="btn btn-warning btn-sm">Ok</a>
                                    </center>
                                ';
                            }
                        ?>
